...
const scale = new itowns.THREE.Vector3(1, 1, 1).divideScalar(extent.dimensions().x);
...
const parent = new itowns.THREE.Mesh(
...
for (let index=0; index<wmsLayers.length; index++) {
    const wms = wmsLayers[index];
    const obj = new itowns.THREE.Object3D();
    // obj.position.add(cubeTransformations[index].position);
    const offset = extent.center().xyz().negate().applyEuler(cubeTransformations[index].rotation);
    ...
    const tileLayer = new itowns.createPlanarLayer('planar' + wms + index, extent, { object3d: obj });
    tileLayer.disableSkirt = true;

    itowns.View.prototype.addLayer.call(view, tileLayer);
...
// view.addFrameRequester(revolution);

let controls = new THREE.OrbitControls(view.camera.camera3D, viewerDiv);
controls.minDistance = 1;
controls.addEventListener('change', () => { view.notifyChange(true); } );
