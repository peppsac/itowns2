...
export function readFeature(crsIn, crsOut, json, filteringExtent, options) {
    const feature = {};

    if (options.filter && !options.filter(json.properties, json.geometry, options.mapboxLayers)) {
        return;
    }
...
export function readFeatureCollection(crsIn, crsOut, json, filteringExtent, options) {
    const collec = [];
