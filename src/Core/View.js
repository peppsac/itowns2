...
import WMTS_Provider from './Scheduler/Providers/WMTS_Provider';
import WMS_Provider from './Scheduler/Providers/WMS_Provider';
import TileProvider from './Scheduler/Providers/TileProvider';
...
function View(crs, viewerDiv, mainLoop) {
    ...
    if (__DEBUG__) {
        this._debug = new Debug(this, viewerDiv);
    }
...
function _preprocessLayer(view, layer, provider) {
    ...
    if (provider instanceof TileProvider || !provider) {
        // const threejsLayer = view.mainLoop.gfxEngine.getUniqueThreejsLayer();
        // layer.threejsLayer = threejsLayer;
        // enable by default
        // TODO camera.camera3D.layers.enable(threejsLayer);
    } else {
...
View.prototype.initDefaultProviders = function initDefaultProviders(scheduler) {
    // Register all providers
    var wmtsProvider = new WMTS_Provider();
    scheduler.addProtocolProvider('wmts', wmtsProvider);
    scheduler.addProtocolProvider('wmtsc', wmtsProvider);
    scheduler.addProtocolProvider('tile', new TileProvider());
    scheduler.addProtocolProvider('wms', new WMS_Provider());
};
 ...
 */
View.prototype.currentCamera = function currentCamera() {
    return this.camera;
};
