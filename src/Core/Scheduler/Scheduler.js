...
Scheduler.prototype.execute = function execute(command) {
    ...
    command.urlInCache = isInCache(command);

    if (command.urlInCache) {
        q.counters.pending++;
...
Scheduler.prototype.flush = function flush(queue) {
    for (const store of queue.storages.values()) {
        for (let i = 0; i < store.q.priv.data.length; i++) {
            const cmd = store.q.priv.data[i];
            if (isInCache(cmd)) {
                console.log(`${(Date.now() - cmd.timestamp) * 0.001} sec`);
